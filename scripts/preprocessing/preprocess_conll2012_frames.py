...
def read_frames(path: str, pos_tag: str, verbal_predicates: Dict = None):
    ...
    for filename in sorted(os.listdir(path)):
        if not filename.endswith('.xml') or f'-{pos_tag}.' not in filename:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conll2012_frames', type=str, default='data_share/conll2012/frames/frame_files')
    parser.add_argument('--output', type=str, default='data_share/conll2012/frames/conll2012_frames.json')

    args = parser.parse_args()
    frames_path: str = args.conll2012_frames
    output_path: str = args.output

    print(' Reading PropBank frames...')
    frames = read_frames(frames_path, 'v')
